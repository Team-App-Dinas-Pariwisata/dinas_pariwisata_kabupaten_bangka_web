...
export async function ApplicantOverview({ userId, userName }: { userId: number; userName: string }) {
  ...
  const mine = (rows: Row[]) => rows.filter((r) => Number(r.created_by) === userId);

  const items: SubmissionItem[] = [
    ...mine(ekrafRows).map((row) => {
      const status = row.status || "Menunggu";
      return {
        id: Number(row.id),
        type: "ekraf" as const,
        typeLabel: "Pelaku Ekraf",
        title: row.nama_usaha || "Pengajuan Ekraf",
        noRegistrasi: row.no_registrasi || "—",
        status,
        createdAt: String(row.created_at ?? ""),
        canEdit: canEditStatus(status),
      };
    }),
    ...mine(sdmRows).map((row) => {
      const status = row.status_pengajuan || "Menunggu";
      return {
        id: Number(row.id),
        type: "sdm" as const,
        typeLabel: "SDM Pariwisata",
        title: row.tempat_bertugas || row.nama_lengkap || "Pengajuan SDM",
        noRegistrasi: row.no_registrasi || "—",
        status,
        createdAt: String(row.created_at ?? ""),
        canEdit: canEditStatus(status),
    ...
    ...mine(komunitasRows).map((row) => {
      const status = row.status_pengajuan || "Menunggu";
      return {
        id: Number(row.id),
        type: "komunitas" as const,
        typeLabel: "Komunitas",
        title: row.nama_organisasi || "Pengajuan Komunitas",
        noRegistrasi: row.no_registrasi || "—",
        status,
        createdAt: String(row.created_at ?? ""),
        canEdit: canEditStatus(status),
  ...
  return (
    <section className="applicant-dashboard">
      <div className="applicant-page-head">
        ...
        <Link href="/akun/pengajuan/pelaku-ekraf" className="applicant-primary">
          <PortalIcon name="plus" /> Buat Pengajuan
        </Link>
      </div>
      ...
      <div className="applicant-stat-grid">
        ...
        <article>
          ...
          <small>Menunggu / perlu perbaikan</small>
        </article>
        <article>
          <span>Disetujui</span>
      ...
      <section className="applicant-new-submission">
        <div className="applicant-section-title">
          <div>
            ...
            <h2>Pilih kategori data yang akan diajukan.</h2>
          </div>
        </div>
        <div className="applicant-service-grid">
          <Link href="/akun/pengajuan/pelaku-ekraf">
            <span className="service-icon"><PortalIcon name="clipboard" /></span>
            <div>
              <strong>Pelaku Ekraf</strong>
              <small>Profil pelaku, usaha, subsektor, aktivitas, dan dokumen.</small>
            </div>
            <PortalIcon name="chevron" />
          </Link>
          <Link href="/akun/pengajuan/sdm-pariwisata">
            <span className="service-icon"><PortalIcon name="users" /></span>
            <div>
              <strong>SDM Pariwisata</strong>
              <small>Identitas, pekerjaan, tempat bertugas, dan sertifikat.</small>
            </div>
            <PortalIcon name="chevron" />
          </Link>
          <Link href="/akun/pengajuan/komunitas">
            <span className="service-icon"><PortalIcon name="database" /></span>
            <div>
              <strong>Komunitas / Asosiasi / Lembaga</strong>
              <small>Profil organisasi, kepengurusan, lokasi, dan legalitas.</small>
            </div>
            <PortalIcon name="chevron" />
          </Link>
        </div>

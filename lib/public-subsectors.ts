...
export async function getPublicSubsectors(): Promise<PublicSubsector[]> {
  const [subsectors, ekraf] = await Promise.all([
...
export async function getPublicSubsector(id: number): Promise<PublicSubsector | null> {
  const row = await getById<DbRecord>("master_subsektor_ekraf", id);
  if (!row || !isTruthyDb(row.aktif)) return null;
  ...
  return {
    ...
    nama_subsektor: String(row.nama_subsektor ?? ""),
    deskripsi: text(row.deskripsi),
    pelaku_count: ekraf.filter((p) => String(p.status ?? "") === "Disetujui" && Number(p.subsektor_id) === id).length,
  };
}
...
export async function getApprovedEkrafBySubsector(subsectorId: number, requestedPage = 1, perPage = 9) {
  ...
  const [all, subsectors, kecamatans, kelurahans] = await Promise.all([
    getAll<DbRecord>("pengajuan_ekraf"),
    getAll<DbRecord>("master_subsektor_ekraf"),
    getAll<DbRecord>("master_kecamatan"),
    getAll<DbRecord>("master_kelurahan"),
  ]);
  const subMap = new Map(subsectors.map((r) => [Number(r.id), String(r.nama_subsektor ?? "")]));
  const kecMap = new Map(kecamatans.map((r) => [Number(r.id), String(r.nama_kecamatan ?? "")]));
  const kelMap = new Map(kelurahans.map((r) => [Number(r.id), String(r.nama_kelurahan ?? "")]));

  const rows = all
    .filter((p) => String(p.status ?? "") === "Disetujui" && Number(p.subsektor_id) === subsectorId)
    .sort(
      (a, b) =>
        Number(b.unggulan ?? 0) - Number(a.unggulan ?? 0) ||
        toTime(b.tanggal_verifikasi) - toTime(a.tanggal_verifikasi) ||
  ...
  const items = rows.slice(offset, offset + safePerPage).map((row) =>
    exposeEkrafImage({
      id: Number(row.id),
      title: String(text(row.nama_merek) ?? text(row.nama_usaha) ?? row.nama_lengkap ?? "Pelaku Ekraf"),
      subtitle: text(row.nama_lengkap),
      category: subMap.get(Number(row.subsektor_id)) ?? null,
      location: kelMap.get(Number(row.kelurahan_usaha_id)) ?? kecMap.get(Number(row.kecamatan_usaha_id)) ?? text(row.alamat_usaha),
      description: text(row.deskripsi_usaha) ?? text(row.produk_jasa),
      image: text(row.file_logo_usaha) ?? text(row.file_foto_dokumentasi) ?? text(row.file_foto_diri),

...
function requiredMissing(form: FormData, field: SubmissionField, existing?: ApplicantSubmissionRow | null) {
  ...
  if (field.type === "file") {
    ...
    return !((file instanceof File && file.size > 0) || Boolean(existing?.[field.key]));
  }
...
function normalizeFieldValue(form: FormData, field: SubmissionField): string | number | null {
  ...
  return ["number", "year"].includes(field.type ?? "") ? Number(value) : value;
}

async function getOwnedSubmission(type: SubmissionType, id: number, userId: number) {
  const row = await getById<ApplicantSubmissionRow>(submissionConfigs[type].table, id);
  return row && Number(row.created_by) === userId ? row : null;
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ data: { ...row, status_label: status, can_edit: canApplicantEdit(status) } });
  } catch (error) {

...
async function requireStaff(request: NextRequest) {
  ...
  if (!user || !["admin", "petugas", "pengguna"].includes(user.role)) return null;
  return user;
}
...
function readKey(conversationId: number, userId: number) {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [rawConversations, rawMessages] = await Promise.all([
      getAll<ConversationRow>("chat_conversations"),
      getAll<MessageRow>("chat_messages"),
    ]);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const messages = (await getAll<MessageRow>("chat_messages")).filter(
      (row) => Number(row.conversation_id) === conversationId && Number(row.id) <= requestedReadId,
    );
    ...
    await setByKey("chat_staff_reads", key, {
      conversation_id: conversationId,
      user_id: user.id,
      last_read_message_id: finalId,
      last_read_at: dbNow(),
    });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!(await getById("chat_conversations", conversationId))) {
    ...
    return NextResponse.json(
      { data: { id, conversation_id: conversationId, sender_type: "staff", sender_user_id: user.id, sender_name_snapshot: user.name, message } },
      { status: 201 },
    );
  } catch (error) {

...
export async function GET(request: NextRequest) {
  ...
  if (!type || !isPublicDirectoryType(type) || !id) {
    return NextResponse.json({ message: "Gambar direktori tidak ditemukan." }, { status: 404 });
  }

  try {
    const refs = await getApprovedDirectoryImageReferences(type, id);
    const key = refs?.map((ref) => keyFromR2SubmissionStorageReference(ref)).find(Boolean) ?? null;
    if (!key) return NextResponse.json({ message: "Gambar direktori tidak ditemukan." }, { status: 404 });

    const object = await getSubmissionFileFromR2(key);
    if (!object) return NextResponse.json({ message: "Gambar direktori tidak ditemukan." }, { status: 404 });
    ...
    return NextResponse.json({ message: "Gambar direktori belum dapat dimuat." }, { status: 502 });
  }
}

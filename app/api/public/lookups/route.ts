...
export async function GET() {
  try {
    ...
    const subsektor = active(subsektorRaw)
      .sort((a, b) => alpha(a.nama_subsektor, b.nama_subsektor))
      .map((r) => ({ value: Number(r.id), label: r.nama_subsektor }));
    ...
    const kelurahan = [...kelurahanRaw]
      ...
      .map((r) => ({
        value: Number(r.id),
        kecamatan_id: Number(r.kecamatan_id),
        label: `${r.jenis ?? ""} ${r.nama_kelurahan ?? ""}`.trim(),
      }));
    const komunitas = active(komunitasRaw)
      .sort((a, b) => alpha(a.nama_komunitas, b.nama_komunitas))
      .map((r) => ({ value: Number(r.id), label: r.nama_komunitas }));

    return NextResponse.json({ data: { subsektor, kecamatan, kelurahan, komunitas } });
  } catch (error) {
    console.error("Public lookups error:", error);
    return NextResponse.json(
      { message: "Pilihan data belum dapat dimuat. Periksa koneksi Firebase Realtime Database." },
      { status: 500 },
    );
  }
}

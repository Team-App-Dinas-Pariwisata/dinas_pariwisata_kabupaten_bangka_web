...
export async function GET(request: NextRequest) {
  ...
  try {
    const [ekrafRows, sdmRows, communityRows, beritaRows, acaraRows] = await Promise.all([
      getAll("pengajuan_ekraf"),
      getAll("pengajuan_sdm_pariwisata"),
      getAll("pengajuan_komunitas_asosiasi"),
      getAll("berita"),
      getAll("acara"),
    ]);
    ...
    const recent: RecentRow[] = [
      ...ekrafRows
        .filter((row) => ["Menunggu", "Perlu Perbaikan"].includes(String(row.status ?? "Menunggu")))
        .map((row) => ({
          id: Number(row.id),
          jenis: "ekraf" as const,
          no_registrasi: row.no_registrasi ? String(row.no_registrasi) : null,
          nama: String(row.nama_lengkap ?? ""),
          detail: String(row.nama_usaha ?? ""),
          status: String(row.status ?? "Menunggu"),
          created_at: String(row.created_at ?? ""),
        })),
      ...sdmRows
        .filter((row) => ["Menunggu", "Perlu Perbaikan"].includes(String(row.status_pengajuan ?? "Menunggu")))
        .map((row) => ({
          id: Number(row.id),
          jenis: "sdm" as const,
          no_registrasi: row.no_registrasi ? String(row.no_registrasi) : null,
          nama: String(row.nama_lengkap ?? ""),
          detail: String(row.tempat_bertugas ?? ""),
          status: String(row.status_pengajuan ?? "Menunggu"),
          created_at: String(row.created_at ?? ""),
        })),
      ...communityRows
        .filter((row) => ["Menunggu", "Perlu Perbaikan"].includes(String(row.status_pengajuan ?? "Menunggu")))
        .map((row) => ({
          ...
          no_registrasi: row.no_registrasi ? String(row.no_registrasi) : null,
          nama: String(row.nama_organisasi ?? ""),
          detail: String(row.kategori ?? ""),
          status: String(row.status_pengajuan ?? "Menunggu"),
          created_at: String(row.created_at ?? ""),
        })),
    ]
    ...
    return NextResponse.json({
      data: {
        ...
        ekraf: ekraf.total,
        sdm: sdm.total,
        komunitas: komunitas.total,
        berita: beritaRows.length,
        acara: acaraRows.length,
        recent,
      },
    });
  } catch (error) {
    ...
    return NextResponse.json(
      {
        message: "Ringkasan dashboard gagal dimuat dari Firebase Realtime Database.",

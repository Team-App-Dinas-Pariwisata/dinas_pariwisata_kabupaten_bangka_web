...
async function preferredOperationalRole() {
  ...
  if (type.includes("'petugas'")) return "petugas";
  throw new Error("Role petugas/operator tidak tersedia pada tabel petugas.");
}

function dbMessage(error: unknown) {
  const code = (error as { code?: string })?.code;
  if (code === "ER_DUP_ENTRY") return "Email atau nomor telepon sudah digunakan.";
  if (code === "ER_ROW_IS_REFERENCED_2") return "Petugas masih terhubung dengan data lain dan tidak dapat dihapus.";
  return error instanceof Error && error.message.includes("Role petugas") ? error.message : "Operasi petugas gagal diproses.";
}

export async function GET(request: NextRequest) {
  if (!(await adminOnly(request))) return NextResponse.json({ message: "Akses ditolak." }, { status: 403 });
  const [rows] = await db().execute<UserRow[]>(
    "SELECT id, role, name, email, phone, status, last_login_at, created_at FROM pengguna WHERE role IN ('operator','verifikator','petugas') ORDER BY created_at DESC",
  );
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ message: "Petugas berhasil dibuat.", id: result.insertId }, { status: 201 });
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const roleFilter = "role IN ('operator','verifikator','petugas')";
    if (password) {
      await db().execute(
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const [result] = await db().execute<ResultSetHeader>(
      "DELETE FROM pengguna WHERE id = ? AND role IN ('operator','verifikator','petugas')",
      [id],
    );
    if (!result.affectedRows) return NextResponse.json({ message: "Petugas tidak ditemukan." }, { status: 404 });

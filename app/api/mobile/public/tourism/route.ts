...
export async function GET(request:NextRequest){
 ...
 const page=Number(request.nextUrl.searchParams.get("page")||1);const size=Number(request.nextUrl.searchParams.get("pageSize")||12);return NextResponse.json({data:await getPublicTourismList(kind,page,size,500)});

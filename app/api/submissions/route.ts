...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    // Notifikasi internal ke admin dan pengaju (best-effort)
    try {
      const tbl = type === "ekraf" ? "pengajuan_ekraf" : type === "sdm" ? "pengajuan_sdm_pariwisata" : "pengajuan_komunitas_asosiasi";
      const [regRows] = await db().execute<(RowDataPacket & { no_registrasi: string | null; created_by: number | null })[]>(
        `SELECT no_registrasi, created_by FROM ${tbl} WHERE id = ? LIMIT 1`, [id],
      );
      ...
      void notifyVerificationAction({
        ...
        noRegistrasi: regRow?.no_registrasi,
      });

      if (regRow?.created_by) {
        void notifyApplicantDecision({
          applicantUserId: regRow.created_by,
          type: type as SubmissionType,
          submissionId: id,
          status,
          note: note || undefined,
